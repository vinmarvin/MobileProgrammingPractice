      ...
      builder: (ctx) => WillPopScope(
        ...
        child: AlertDialog(
          ...
          content: const Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            SizedBox(height: 12),
            Text('🌿 Ambil jeda 20–30 menit sekarang.', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.primaryDark)),
          ]),
          ...
          actions: [
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                style: ElevatedButton.styleFrom(backgroundColor: AppColors.primaryDark, foregroundColor: AppColors.white, padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                onPressed: () {
                  Navigator.pop(ctx);
              ...
              flexibleSpace: FlexibleSpaceBar(
                background: Container(
                  decoration: const BoxDecoration(
                    gradient: LinearGradient(colors: [AppColors.primaryDarker, AppColors.primaryLight], begin: Alignment.topLeft, end: Alignment.bottomRight),
                  ),
                  child: Padding(
                      ...
                      children: [
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              const Text('Selamat datang,', style: TextStyle(color: AppColors.white70, fontSize: 14)),
                              Text(user?.name ?? '...', style: const TextStyle(color: AppColors.white, fontSize: 22, fontWeight: FontWeight.bold), maxLines: 1, overflow: TextOverflow.ellipsis),
                            ],
                          ),
                        ),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
                          decoration: BoxDecoration(color: AppColors.white.withOpacity(0.2), borderRadius: BorderRadius.circular(20), border: Border.all(color: AppColors.white.withOpacity(0.4))),
                          child: Row(children: [
                            const Text('⭐', style: TextStyle(fontSize: 16)),
                            const SizedBox(width: 6),
                            Text('${user?.points ?? 0} pts', style: const TextStyle(color: AppColors.white, fontWeight: FontWeight.bold)),
                          ]),
                        ),
                      ],
                    ),
...
class _StatsRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(children: [
      Expanded(child: _StatCard(label: 'Total Poin', value: '${user?.points ?? 0}', icon: '⭐', color: AppColors.backgroundLightOrange, textColor: AppColors.textOrange)),
      const SizedBox(width: 12),
      Expanded(child: _StatCard(label: 'Menit Fokus', value: '${user?.totalFocusMinutes ?? 0}', icon: '⏱️', color: AppColors.backgroundLightBlue, textColor: AppColors.textDarkBlue)),
      const SizedBox(width: 12),
      Expanded(child: _StatCard(label: 'Role', value: user?.role ?? '-', icon: '🏅', color: AppColors.backgroundLightPurple, textColor: AppColors.textDarkPurple)),
    ]);
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    if (isRest) {
      ...
      switch (timer.statusLabel) {
        ...
        case 'long_break': ringColor = AppColors.primaryDarker; break;
        default: ringColor = AppColors.primary;
      }
    }
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          if (!isIdle) ...[
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              ...
              Text('${(timer.sessionProgress * 100).toInt()}%', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: AppColors.primaryDark)),
            ]),
            const SizedBox(height: 4),
            ClipRRect(
              ...
              Column(children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: isRest ? Colors.teal.shade50 : AppColors.backgroundGreenLight,
                    borderRadius: BorderRadius.circular(10),
                  ),
                  child: Text(
                    isRest ? '😌 REST' : '🎯 FOKUS',
                    style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: isRest ? Colors.teal.shade700 : AppColors.primaryDark),
                  ),
                ),
                const SizedBox(height: 4),
                Text(
          ...
          if (isIdle) ...[
            ...
            Wrap(
              ...
              children: [1, 2, 3, 4, 6, 8].map((h) {
                final min = h * 60;
                final isSelected = timer.targetMinutes == min;
                return ChoiceChip(
                  label: Text('$h jam'),
                  selected: isSelected,
                  selectedColor: AppColors.primary,
                  labelStyle: TextStyle(color: isSelected ? AppColors.white : null, fontSize: 12),
                  onSelected: (_) => context.read<TimerProvider>().setTarget(min.toDouble()),
                );
              }).toList(),
            ),
          ],
            ...
            if (isStopped || isRunning) ...[
              ...
              Expanded(
                child: ElevatedButton.icon(
                  ...
                  style: ElevatedButton.styleFrom(backgroundColor: Colors.green.shade700, foregroundColor: AppColors.white),
                  onPressed: () async {
          ...
          Padding(
            ...
            child: Text(
              ...
              style: TextStyle(color: isRest ? Colors.teal.shade600 : AppColors.primaryDark, fontSize: 12, fontWeight: FontWeight.w500),
            ),
          ),
        ]),
      ),
    ...
    return Container(
      decoration: BoxDecoration(
        gradient: const LinearGradient(
          colors: [AppColors.darkThemeBg1, AppColors.darkThemeBg2, AppColors.darkThemeBg3],
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Radio Musik Fokus', style: TextStyle(color: AppColors.white, fontWeight: FontWeight.bold, fontSize: 15)),
              Text(isPlaying ? '▶ Sedang diputar' : '■ Siap diputar', style: TextStyle(color: isPlaying ? AppColors.primary : AppColors.white54, fontSize: 11)),
            ]),
            const Spacer(),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
              decoration: BoxDecoration(
                color: isPlaying ? AppColors.primary.withOpacity(0.2) : AppColors.white.withOpacity(0.1),
                borderRadius: BorderRadius.circular(20),
                border: Border.all(color: isPlaying ? AppColors.primary : AppColors.white30),
              ),
              child: Text(isPlaying ? 'LIVE' : 'OFF', style: TextStyle(color: isPlaying ? AppColors.primary : AppColors.white54, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1)),
            ),
          ]),
          ...
          Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(vibe['label']!, style: const TextStyle(color: AppColors.white, fontWeight: FontWeight.bold, fontSize: 18)),
              const SizedBox(height: 2),
              Text('Focus • Loop mode aktif', style: TextStyle(color: AppColors.white60, fontSize: 12)),
              const SizedBox(height: 6),
              ...
              if (isPlaying)
                ...
                Text('Tekan ▶ untuk mulai', style: TextStyle(color: AppColors.white38, fontSize: 11)),
            ])),
            // Controls
            Column(children: [
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Icon(isPlaying ? Icons.pause : Icons.play_arrow, color: AppColors.white, size: 28),
                ),
              ),
            ]),
          ]),
          ...
          // Vibe selector
          const Text('PILIH VIBE', style: TextStyle(color: AppColors.white38, fontSize: 10, letterSpacing: 2, fontWeight: FontWeight.bold)),
              ...
              itemBuilder: (ctx, i) {
                ...
                return GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      color: isSelected ? AppColors.primary.withOpacity(0.2) : AppColors.white.withOpacity(0.07),
                    ...
                    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      ...
                      const SizedBox(height: 2),
                      Text(v['label']!, style: TextStyle(color: isSelected ? AppColors.primary : AppColors.white60, fontSize: 10, fontWeight: isSelected ? FontWeight.bold : FontWeight.normal)),
                    ]),
                  ),
                );
              },
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(14),
          border: Border.all(color: isSelected ? AppColors.primaryDark : AppColors.transparent, width: 2),
          boxShadow: [BoxShadow(color: AppColors.black.withOpacity(0.08), blurRadius: 8, offset: const Offset(0, 4))],
        ),
        child: ClipRRect(
          ...
          child: Stack(children: [
            Image.network(
              ...
              errorBuilder: (_, __, ___) => Container(height: 140, color: AppColors.backgroundGreenLight, alignment: Alignment.center, child: const Icon(Icons.play_circle_fill, color: AppColors.primaryDark, size: 40)),
            ),
            Positioned.fill(child: Container(color: AppColors.black.withOpacity(0.25))),
            Positioned(left: 8, top: 8, child: Container(
              ...
              child: Text(video['category']!, style: const TextStyle(color: AppColors.white, fontSize: 9, fontWeight: FontWeight.w600)),
            )),
            Positioned(right: 8, bottom: 8, child: Container(
              padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 3),
              decoration: BoxDecoration(color: AppColors.black54, borderRadius: BorderRadius.circular(20)),
              child: Text(video['duration']!, style: const TextStyle(color: AppColors.white, fontSize: 9, fontWeight: FontWeight.w600)),
            )),
            ...
            Positioned.fill(child: Center(child: Icon(Icons.play_circle_outline, color: AppColors.white.withOpacity(0.85), size: 36))),
          ]),
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: AppColors.black.withOpacity(0.06), blurRadius: 10, offset: const Offset(0, 4))],
      ),
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Row(children: [
          ClipRRect(
            ...
            child: Image.network(
              ...
              errorBuilder: (_, __, ___) => Container(width: 80, height: 55, color: AppColors.backgroundGreenLight, child: const Icon(Icons.play_circle_fill, color: AppColors.primaryDark)),
            ),
          ),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.red.shade600, foregroundColor: AppColors.white,
          ...
          child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(color: AppColors.white.withOpacity(0.3), borderRadius: BorderRadius.circular(6)),
                child: Text(article['category']!, style: const TextStyle(color: AppColors.white, fontSize: 10, fontWeight: FontWeight.w600)),
              ),
              const SizedBox(height: 6),
              Text(article['title']!, style: const TextStyle(color: AppColors.white, fontWeight: FontWeight.bold, fontSize: 13), maxLines: 3, overflow: TextOverflow.ellipsis),
            ]),
          ),
        ),
      ),
